"""SVM wrapper supporting both classifier and regressor modes."""
...
class SVMModel(BaseModel):
    """Wrapper around sklearn SVC (probability=True) / SVR.

    Parameters
    ----------
    params : dict | None
        Forwarded to the underlying estimator (e.g. C, kernel, gamma).
    mode : str
    ...
    def _build_model(self) -> None:
        if self._mode == "classifier":
            ...
            self._model = SVC(probability=True, **self.params)
        else:
    ...
    def feature_importances(self) -> np.ndarray | None:
        """SVM does not natively provide feature importances; return None."""
        return None
