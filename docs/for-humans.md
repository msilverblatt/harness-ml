...
**harness-plugin** is built on [protomcp](https://github.com/msilverblatt/protomcp). Each handler is a `@tool_group` class with `@action` methods, giving the LLM clean per-action schemas instead of a monolithic parameter blob. The experiment lifecycle uses a protomcp `@workflow` with dynamic tool visibility — the agent can only see valid next steps at each point, structurally enforcing experiment discipline. Cross-cutting concerns (error formatting, event emission, Studio lifecycle) are handled by protomcp middleware and telemetry sinks.
