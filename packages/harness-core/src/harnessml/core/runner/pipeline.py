...
ProjectConfig and delegating to harnessml-models components.  Supports
...
class PipelineRunner:
    ...
    def _normalize_columns(self) -> None:
        """Auto-detect domain-specific columns and rename to harnessml convention.

        Applies renames from two sources:
        1. Config-level column_renames (from pipeline.yaml data section)
        2. Hook-registered renames (from plugins like harness-sports)
    ...
    def _compute_backtest_metrics(
        ...
        Uses BacktestRunner from harnessml-models for binary tasks,
